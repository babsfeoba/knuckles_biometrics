...
for i in range(1, test_no + 1, 1):
    img = cv2.imread(
        "C:/Users/usr310/Documents/Knuckles/Major/Gen_Test/"+str(i)+".bmp")
...
for i in range(1, train_no + 1, 1):
    img = cv2.imread(
        "C:/Users/usr310/Documents/Knuckles/Major/Gen_Train/"+str(i)+".bmp")
    res = cv2.resize(img, dim, interpolation=cv2.INTER_AREA)
...
model = Sequential()
model.add(Conv2D(96, kernel_size=(11, 11), activation='relu',
...
history = model.fit(X_train, y_train, validation_split = 0.25, epochs=40, batch_size=30)
...
file = open("y_pred_ANori_major", "wb")
pickle.dump(y_pred, file)
file.close()
file = open("y_test_ANori_major", "wb")
pickle.dump(y_test, file)
file.close()


file = open("trainHistoryANori_major", "wb")
pickle.dump(history, file)
file.close()

from keras.models import load_model
model.save('modelOriMajor.h5')
model_final = load_model('modelOriMajor.h5')
